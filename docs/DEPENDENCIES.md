...
| **`treehouse`** | Optional | Worktree pool for parallel isolation. `setup-harness.ts` seeds `treehouse.toml` (`max_trees = 16`, `root = ".tmp/treehouse/"`). Readiness checks it only when isolation is required; `-PrepareIsolation -Parallel` leases explicitly. | `treehouse --version` → `v1.8.0`. Try from the project root: `treehouse status` | Ordinary in-session work remains available. When isolation is required, readiness fails fast with remediation instead of falling back to the main worktree. The seeded `treehouse.toml` sits inert when the tool is absent. |
